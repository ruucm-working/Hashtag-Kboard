...
if(!class_exists('HashtagPluginOption')) {
	class HashtagPluginOption extends EasyOptions {
		...
		function hashtag_regex_replace( $content ) {
			...
			$content = preg_replace('/(?<!:|\s|&|"|\')(\s)#([^\s<]+)/i', '<span class="'.$class.'">\1#<a href="'.site_url().'/'.$this->get_current_board_name().'?mod=list&pageid=1&target=&keyword=&tag='.$hash.'\2">\2</a></span>', $content);
			return $content;
		}
		...
		function hashtag_first_line( $string ) {
			...
			if (substr($string, 0, 1) === $htag) {
				$arr = explode(" ", $string);
				$arr[0] = str_replace("<br","", $arr[0]);
				$arr[1] = str_replace("/>","", $arr[1]);
				$arr[0] = '<span class="'.$class.'"><a href="'.site_url().'/'.$this->get_current_board_name().'?mod=list&pageid=1&target=&keyword=&tag='.str_replace("#","%23", $arr[0]).'">'.$arr[0].'</a></span>';
				$string = implode(" ", $arr);
			}
			return $string;
		}

		function get_current_board_name() {
			$current_url = get_permalink();
			$board_name = explode("/", $current_url)[3];
			return $board_name;
		}
